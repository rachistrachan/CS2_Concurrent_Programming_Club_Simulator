...
public class ClubGrid {
	...
	public GridBlock enterClub(PeopleLocation myLocation) throws InterruptedException {
		...
		synchronized (entranceAccessLock) {
			while (counter.overCapacity() || (!entrance.get(myLocation.getID()))) { // checks capacity of club and if entrance is open
				entranceAccessLock.wait(); // wait until conditions change
			}
			if (!counter.overCapacity() && entrance.get(myLocation.getID())) {
	...
	public GridBlock move(GridBlock currentBlock,int step_x, int step_y,PeopleLocation myLocation) throws InterruptedException {  //try to move in
		...
		if (!newBlock.occupied() && newBlock.get(myLocation.getID())) {
	...
	public  void leaveClub(GridBlock currentBlock,PeopleLocation myLocation)   { // edit!! no one is leaving
		synchronized (entranceAccessLock){
